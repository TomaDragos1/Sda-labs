...
int cmp_number_letters(void *el1, void *el2, void *name)
{
...
void ord_frequence(TLG *al, TLG element_search)
{
	...
	{	
		...
		if(el_search->frequency > el_crt->frequency)
		{
			break;
		}
	}
...
void free_inf_second(void *elem_info)
{
...
void eliberare_second_list(TLG *element)
{
	...
	//printf("acesta este:%d\n", info1->number_letters);
	...
	while(getline(&line, &len, f) != -1){
		...
		if(strcmp(word, "insert") == 0)
		{
			...
			while(word != NULL)
			{
				if(strlen(word) >= 3)
				{
					TLetters *crt_word_cel =(TLetters*)malloc(sizeof(TLetters)); 
					if(crt_word_cel == NULL)
					return NULL;
					crt_word_cel->number_letters = strlen(word);
					crt_word_cel->head_list = NULL;
					rez = InsTH(h, crt_word_cel, cmp_number_letters, word, insert_second_list, cmp_first_list);
					if(rez == 0)
					return NULL;
				}
				word = strtok(NULL, " ,.\n\r");
