...
int InsTH(TH*a, void* ae, TFCmp fcmp, void *name, TFinsertsec afinsert, TFcompare firs_list_cmp)
{
    int cod = a->fh(name);
    ...
    for(el = a->v[cod]; el != NULL; el = el->urm) {
        if (fcmp(el->info, ae, name) == 1)
        {   
            afinsert(&el, name);
            return 1;
        }
    }
    TLG rez = Ins_IncLG(a->v+cod, ae, firs_list_cmp); /* reminder: a->v+cod <=> &a->v[cod] */
